...
def main():
    ...
    if utils.is_primary(args) and args.log_wandb:
        if has_wandb:
            wandb.init(project='overfit', config=args)
        else:
            _logger.warning(
...
def train_one_epoch(
    ...
    for batch_idx, minibatch in enumerate(loader):
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            # =========================================================================================
            positive = 0  # y_true must be your labels
            for y in target:
                # print(str(y))
                if (y == 1):
                    positive += 1
            ...
            print("positive label count:" + str(positive))
            print("total label count:" + str(len(target)))
            # ROC
            # wandb.log({"ROC": wandb.plot.roc_curve(ground_truth.detach(), predictions.detach())})
            # Precision-Recall
            # wandb.log({"pr": wandb.plot.pr_curve(ground_truth.detach(), predictions.detach(), labels=None, classes_to_plot=None)})
            # Confusion Matrices
